...
- The friction surface reports patterns rather than actions, so it renders under Charted Next with all three counts (surfaced, suppressed, unclassified) stated even when every one is zero; only a signature actually waiting on the captain's triage decision moves to Captain's Call, and `friction-triage` owns everything else about it.
