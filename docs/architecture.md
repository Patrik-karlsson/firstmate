...
For whole-fleet read-only review, `bin/fm-fleet-snapshot.sh --json` emits schema `fm-fleet-snapshot.v1` from the backlog, task metadata, current crew state, endpoint probes, PR/report pointers, scout reports, this home's durable friction records, bounded current summaries from registered secondmate homes, and secondmate return-channel guidance.
...
The non-blocking `friction:` verb is transparent to every state reader and is absorbed as its own signal class: because change detection is a size:mtime signature, a friction append would otherwise wake firstmate exactly like a real status, so the watcher folds only the bytes appended since the recorded size and absorbs the wake when they are friction and nothing else, failing closed on a mixed delta, a first sighting, or an unreadable read.
...
The same file pins `agent` to the agents that the opt-out can actually be applied to, because no-mistakes refuses to launch a gate agent it cannot strip of `AGENTS.md`; a machine-wide agent without that knob leaves this repo unvalidatable rather than merely less isolated.
The pin is a code-executing selection field, so it is read from the default-branch copy only while `allow_repo_commands` stays false, unlike the opt-out itself, which is trusted unconditionally.
It is an ordered list that resolution filters to the installed agents, and `tests/fm-gate-refuse.test.sh` owns the allowed set and fails on any addition outside it.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which reads the PR's check state from the forge and then records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` before calling `gh-axi pr merge`.
That preflight is what makes "never merge a red PR" mechanical rather than a matter of having looked: a failing or unfinished check state refuses and names the concrete check, an unreadable one refuses rather than merging blind, a repository with no checks configured merges normally because an empty rollup is not red, and the only way past a refusal is an explicit flag carrying the classification of why the check is not passing, recorded as `checks_override=` in the task's meta.
No environment variable or configuration key reaches that flag, so no standing posture can acquire the authority the flag grants.
